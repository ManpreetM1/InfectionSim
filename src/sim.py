...
shopping_infection_prob = min(infection_prob + (0.32 * infection_prob), 1) 
shopping_centres = []
...
if(isolation):
    infection_radius = isolation_infection_radius
...
def update_agents():
    ...
    infected_agents = agents.loc[(agents["state"] == "I") & (agents["time_infected"] >= RECOVERY_TIME)]
    recovery_agents = infected_agents.sample(frac = 0.1, replace = False)
